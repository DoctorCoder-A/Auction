...
rollback:
	ssh ${HOST} -p ${PORT} 'cd site ${BUILD_NUMBER} && docker-compose -f docker-compose-production.yml pull'
	ssh ${HOST} -p ${PORT} 'cd site ${BUILD_NUMBER} && docker-compose -f docker-compose-production.yml up build --remove-orphans -d'
